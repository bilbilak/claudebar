...
enum Command {
    ...
    Tray {
        /// Poll interval in seconds (clamped to 120-3600).
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Command::Tray { interval } => tray::run(interval.clamp(120, 3600)),
    }
}
...
fn signin() -> Result<()> {
    ...
    println!(
        "{}",
        gettext("Signed in; tokens stored in the Secret Service.")
    );
    Ok(())
}
...
fn signout() -> Result<()> {
    ...
    println!(
        "{}",
        gettext("Signed out; tokens removed from the Secret Service.")
    );
    Ok(())
}

fn status() -> Result<()> {
    // `status` emits machine-readable JSON; do NOT translate its output.
