...
function formatReset(d: Date | null): string {
  ...
  if (mins < 60) return fmt(_('in %d min'), mins);
  const hrs = Math.floor(mins / 60);
  const rem = mins % 60;
  if (hrs < 24) return rem ? fmt(_('in %dh %dm'), hrs, rem) : fmt(_('in %dh'), hrs);
  const days = Math.floor(hrs / 24);
  const remH = hrs % 24;
  return remH ? fmt(_('in %dd %dh'), days, remH) : fmt(_('in %dd'), days);
}
...
class ClaudeIndicatorImpl extends PanelMenu.Button {
  ...
  declare _styleChangedId: number;
  ...
  _init(extension: Extension, source: UsageSource, settings: Settings) {
    ...
    this._settingsChangedId = settings.raw().connect('changed', (_s: Gio.Settings, key: string) => {
      this._percentBox.visible = settings.showPercentages;
      this._area.queue_repaint();
      if (key === 'poll-interval-seconds') this._restartPolling();
      if (key === 'auth-changed-tick') {
        this._refresh().catch((e) => logError(e as Error, 'claudebar: auth-poke refresh failed'));
      }
    });

    this._styleChangedId = this.connect('style-changed', () => this._applyLabelColors());
  ...
  _buildMenu() {
    ...
    this._sessionLabel = new St.Label({ text: _('Current session: —'), style_class: 'claudebar-menu-line' });
    ...
    this._weeklyLabel = new St.Label({ text: _('Weekly (all models): —'), style_class: 'claudebar-menu-line' });
  ...
  _startPolling() {
    ...
    const interval = Math.max(120, this._settings.pollInterval);
  ...
  _updateMenuLabels() {
    ...
    if (!s) return;
    this._sessionLabel.text = fmt(_('Current session: %d%%'), Math.round(s.session.percent));
    this._sessionReset.text = fmt(_('Resets %s'), formatReset(s.session.resetsAt));
    this._weeklyLabel.text = fmt(_('Weekly (all models): %d%%'), Math.round(s.weekly.percent));
    this._weeklyReset.text = fmt(_('Resets %s'), formatReset(s.weekly.resetsAt));
