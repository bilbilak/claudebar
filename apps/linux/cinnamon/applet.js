...
// The Makefile install target compiles each po/<lang>.po into
// ~/.local/share/locale/<lang>/LC_MESSAGES/claudebar@bilbilak.org.mo
...
ClaudebarApplet.prototype = {
    ...
    _init: function(metadata, orientation, panelHeight, instance_id) {
        ...
        this._sessionPct = new St.Label({
            text: "",
            style_class: "claudebar-percent-label",
            y_align: Clutter.ActorAlign.CENTER,
        });
        this._weeklyPct = new St.Label({
            text: "",
            style_class: "claudebar-percent-label",
            y_align: Clutter.ActorAlign.CENTER,
        });
        this._percentBox.add_actor(this._sessionPct);
        this._percentBox.add_actor(this._weeklyPct);
        this._percentBox.visible = !!this.showPercentages;
        this._box.add_actor(this._percentBox);

        // Replace default icon with our box.
        this.actor.add_style_class_name("claudebar-applet");
        this.actor.remove_all_children && this.actor.remove_all_children();
        this.actor.add(this._box, { y_align: St.Align.MIDDLE });

        this._styleChangedId = this.actor.connect("style-changed", () => this._applyLabelColors());
    ...
    _buildMenu: function() {
        this._sessionItem = new PopupMenu.PopupMenuItem(_("Current session: —"), { reactive: false });
        ...
        this._weeklyItem = new PopupMenu.PopupMenuItem(_("Weekly (all models): —"), { reactive: false });
        ...
        const open = new PopupMenu.PopupMenuItem(_("Open claude.ai/settings/usage"));
    ...
    _startPolling: function() {
        ...
        const interval = Math.max(120, this.pollInterval || 300);
    ...
    _redraw: function() {
        if (this._percentBox) this._percentBox.visible = !!this.showPercentages;
        this._updatePercentLabels();
        if (this._area) this._area.queue_repaint();
    },

    _updatePercentLabels: function() {
        if (!this._sessionPct || !this._weeklyPct) return;
    ...
    _applyLabelColors: function() {
        if (!this._sessionPct || !this._weeklyPct) return;
        try {
            const fg = this.actor.get_theme_node().get_foreground_color();
            const style = "color: rgba(" + fg.red + ", " + fg.green + ", " + fg.blue + ", 0.92);";
            this._sessionPct.set_style(style);
            this._weeklyPct.set_style(style);
        } catch (_e) {
            /* theme node not realized yet — style-changed will fire again later */
        }
    },
    ...
    _updateMenu: function() {
        const s = this._snapshot;
        if (!s) return;
        const sessPct = s.session.percent.toFixed(0);
        const weekPct = s.weekly.percent.toFixed(0);
        this._sessionItem.label.text = formatTpl(_("Current session: %d%%"), [sessPct]);
        this._sessionReset.label.text = formatTpl(_("Resets %s"), [formatReset(s.session.resetsAt)]);
        this._weeklyItem.label.text = formatTpl(_("Weekly (all models): %d%%"), [weekPct]);
        this._weeklyReset.label.text = formatTpl(_("Resets %s"), [formatReset(s.weekly.resetsAt)]);

        const statusTexts = {
            ok: "",
            offline: _("Offline — last value may be stale"),
            "rate-limited": _("Rate limited by Claude API"),
            unauthenticated: _("Not signed in — open Settings to add a token"),
        };
    ...
    onSignInClicked: function() {
        try {
            ...
            // Cinnamon's settings dialog (xlet-settings) doesn't host
            // arbitrary buttons, so mirror the helper-based UX: silently
            // copy the OAuth link to the clipboard and ping the user via
            // the desktop notification daemon. If the default browser
            // isn't where they're signed in to Claude, they can paste
            // into the right one.
            try {
                const clipboard = St.Clipboard.get_default();
                clipboard.set_text(St.ClipboardType.CLIPBOARD, flow.authorizeUrl);
            } catch (e) { /* best effort */ }
            try {
                // Use the argv form so apostrophes in translated strings
                // can't break shell quoting.
                const proc = new Gio.Subprocess({
                    argv: [
                        ...
                        "--expire-time=4000",
                        "ClaudeBar",
                        _("Sign-in link copied to clipboard"),
                    ],
    ...
    on_applet_removed_from_panel: function() {
        ...
        if (this._styleChangedId && this.actor) {
...
function formatReset(d) {
    ...
    if (ms <= 0) return _("now");
    const mins = Math.round(ms / 60000);
    if (mins < 60) return formatTpl(_("in %d min"), [mins]);
    const hrs = Math.floor(mins / 60);
    const rem = mins % 60;
    if (hrs < 24) return rem
        ? formatTpl(_("in %dh %dm"), [hrs, rem])
        : formatTpl(_("in %dh"), [hrs]);
    const days = Math.floor(hrs / 24);
    const remH = hrs % 24;
    return remH
        ? formatTpl(_("in %dd %dh"), [days, remH])
        : formatTpl(_("in %dd"), [days]);
}

// Minimal printf-style helper: substitutes %d/%s placeholders in order, then
// collapses %% to a literal %. Used so translated strings keep their printf
// format specifiers (which is what xgettext / our regen script expect).
function formatTpl(tmpl, args) {
    let i = 0;
    const replaced = tmpl.replace(/%[ds]/g, () => {
        const v = args[i++];
