...
public sealed class SettingsStore : INotifyPropertyChanged
{
    ...
    private bool _floatingMeterEnabled = true;
    ...
    {
        get => _pollInterval;
        set { if (SetField(ref _pollInterval, Clamp(value, 120, 3600))) Save(); }
    }
    ...
    private static SettingsStore Load()
    {
        try
        {
            ...
            {
                _pollInterval = Clamp(loaded.PollIntervalSeconds ?? 300, 120, 3600),
                _showPercentages = loaded.ShowPercentages ?? true,
                _warnThreshold = Clamp(loaded.WarnThreshold ?? 60, 0, 100),
                _criticalThreshold = Clamp(loaded.CriticalThreshold ?? 85, 0, 100),
                _floatingMeterEnabled = loaded.FloatingMeterEnabled ?? true,
