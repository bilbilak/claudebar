...
public partial class FloatingMeterWindow : Window
{
    ...
    private void ApplySettings(SettingsStore s)
    {
        ...
        var newWidth = Math.Max(120, Math.Min(320, s.FloatingMeterWidth));
        // Anchor the right edge: when the user changes width via Preferences,
        // keep Right (= Left + Width) constant so the widget grows/shrinks
        // toward the left rather than nudging itself off the bottom-right of
        // the screen. Only do this once the window is on screen — on first
        // construction Left isn't set yet (RestorePosition handles initial
        // placement).
        if (IsLoaded && !double.IsNaN(Width) && Math.Abs(Width - newWidth) > 0.5)
        {
