...
User-visible strings are wrapped in `String(localized: "…")` and resolved at
runtime against `Resources/Localizable.xcstrings` (Xcode String Catalog). The
canonical translation source lives at `../../i18n/strings.yaml`; regenerate the
catalog from it with:

```sh
make i18n
```

The committed `Localizable.xcstrings` is an empty placeholder until the regen
script populates it — every `String(localized:)` call falls back to its English
source string until then.
