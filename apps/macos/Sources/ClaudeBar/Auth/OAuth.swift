...
enum OAuthError: LocalizedError {
    ...
    var errorDescription: String? {
        // High-level "Sign-in failed" / "Could not start sign-in" prefixes have
        // catalog entries; the technical detail strings below are intentionally
        // English-only (not in i18n/strings.yaml) — they're developer/diagnostic
        // messages, not localized UI copy.
        switch self {
        ...
        case .signInTimedOut: return "\(String(localized: "Sign-in failed")): timed out."
        case .cancelled: return "\(String(localized: "Sign-in failed")): cancelled."
        case .listenerFailure(let s): return "\(String(localized: "Could not start sign-in")): loopback listener failed: \(s)"
        }
    }
}
